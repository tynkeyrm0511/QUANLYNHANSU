...
namespace NHANSU_UIUX
{
    public partial class frmTonGiao : DevExpress.XtraEditors.XtraForm
    {
        ...
        void loadData()
        {
            gvDanhSach.DataSource = _tongiao.getList();
        }
        void saveData()
        {
        ...
        private void gvDanhSach_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rowIndex >= 0 && rowIndex < gvDanhSach.Rows.Count)
            {
                ...
                _id = Convert.ToInt32(selectedRow.Cells["IDTG"].Value.ToString());
                txtThemSua.Text = selectedRow.Cells["TENTG"].Value.ToString();
            }
        }
    }
}