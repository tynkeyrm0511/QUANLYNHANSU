...
namespace NHANSU_UIUX
{
    public partial class frmNhanVien_ThoiViec : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void _reset()
        {
            ...
            dtpNgayNghi.Value = dtpNgayNopDon.Value.AddDays(45);

        }
        void loadData()
        {
            gvDanhSach.DataSource = _kt.getListFull();
        }
        void saveData()
        {
            tb_NHANVIEN_THOIVIEC kt;
            if (_themhopdong)
            {
                //SOHD :    00001/yyyy/HDLD
                //example:  11548/2024/HDLD
                var maxSoQD = _kt.CurrentSoHD();
                int n = int.Parse(maxSoQD.Substring(0, 5)) + 1;
                kt = new tb_NHANVIEN_THOIVIEC();
                kt.SOQD = n.ToString("00000") + @"/2024/QDKL";
                //hd.NGAYBD = dtpNgayBatDau.Value;
                //hd.NGAYKT = dtpNgayKetThuc.Value;
                kt.NGAYNGHI = dtpNgayNghi.Value;
                kt.NGAYNOPDON = dtpNgayNopDon.Value;
                kt.LYDO = txtLyDo.Text;
                kt.GHICHU = txtNoiDung.Text;
            ...
            var nv = _nhanvien.getItem(kt.MANV.Value);
            nv.DATHOIVIEC = true;
            _nhanvien.Update(nv);
        }
        void loadNhanVien()
        ...
        private void gvDanhSach_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rowIndex >= 0 && rowIndex < gvDanhSach.Rows.Count)
            {
                ...
                _soqd = selectedRow.Cells["SOQD"].Value.ToString();
                ...
                dtpNgayNghi.Value = kt.NGAYNGHI.Value;
                dtpNgayNopDon.Value = kt.NGAYNOPDON.Value;
                sludNhanVien.Text = kt.MANV.ToString();
                txtLyDo.Text = kt.LYDO.ToString();
                txtNoiDung.Text = kt.GHICHU.ToString();

            }
        }

        private void dtpNgayNopDon_ValueChanged(object sender, EventArgs e)
        {
            dtpNgayNghi.Value = dtpNgayNopDon.Value.AddDays(45);
            dtpNgayNghi.Refresh();
        }
